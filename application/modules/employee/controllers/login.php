...
class Login extends MX_Controller {
  ...
  public function index(){
   		...
   		if(isset($_POST['submit']) and $_POST['submit']=='Login'){
       	 $this->form_validation->set_rules('username', 'Email', 'trim|required|min_length[1]|xss_clean|valid_email');
   		 $this->form_validation->set_rules('password', 'Password', 'trim|required|min_length[5]|max_length[255]|xss_clean');
   			if($this->form_validation->run() != false){
   				$parameter = array(
   					                  'p_mode'     => 'Selectlogin',
                                	  'a_EmpId'    => 'null',
             					      't_EmailId'  => $this->input->post('username'),
             						  't_Password' => md5($this->input->post('password'))
           						); 
                // api call comes here
			   	$path  = base_url().'api/employee/emp/format/json/';
			   	$response  = curlcall($parameter, $path);

                if($response =='Something Went Wrong' or $response==""){

   					$this->session->set_flashdata('message','Please check email and password');
   					$base_url  = base_url();
   					redirect($base_url.'employee/login/index');
   					exit();
...
$lastlogin=$response->lastlogin;
					...
					$myarray = array(
   									   'a_SId' 	      => $response->a_EmpId,
   									   'a_SysAdminId' => $response->t_EmpCode,
   									   't_username'   => $response->t_EmaiId,
   									   'lastlogin'    => $lastlogin,
   									   'business_id'  => $response->n_BusinessId,
   									   'firstname'    => $response->t_EmpFirstName,
   									   't_EmpLastName'=> $response->t_EmpLastName,
   									   'n_DeptId'     => $response->n_DeptId,
   									   'n_PolicyId'   => $response->n_PolicyId
   									);

					$this->session->set_userdata('sessionData', $myarray);
					$base_url  = base_url();
   					redirect($base_url.'employee/claim');
   					exit();
   				}
...
public function dmadd(){
   ...
   if(isset($_POST['submit'])){
      $userId= checklogin();
      $businessName=$this->input->post('business_name_id');
      if(!empty($businessName)){
         $this->form_validation->set_rules('business_name_id', 'Distance Measurement Name', 'trim|required|xss_clean|');
        if($this->form_validation->run() != false){
          $businessName=$this->input->post('business_name_id');
        }else{
          $businessName="";
      ...
      $this->form_validation->set_rules('type_id', 'Distance Measurement Name', 'trim|required|xss_clean|');
      $this->form_validation->set_rules('dm_name', 'Distance Measurement Name', 'trim|required|xss_clean|');
      if($this->form_validation->run() != false){

         $dmLabel=$this->input->post('enum_type_id');
         $typeId=$this->input->post('type_id');
         $dmName=$this->input->post('dm_name');
         $enumKey=md5(date('Y:m:d'));
         $parameter = array( 'p_mode' => 'Insert',
                              'a_EnumId' => 'null',
                              't_EnumTypeDescription' => $dmLabel,
                               'enumKey' => $enumKey,
                               'b_ForAll' => $typeId,
      ...
      // exit();
       ...
       if($response =='Something Went Wrong'){
                ...
                $data['listing']=$response;
                $this->load->view('layout/header');
                $this->load->view('setting/dmAdd',$data);
               // $this->load->view('layout/footer');                     
            }
...
public function editdm(){
  ...
  if(isset($_POST['submit'])){
      ...
      if($this->form_validation->run() != false){
         $dmName=$this->input->post('dm_name');
         $dmId=$this->input->post('dm_id');
         $enumKey=md5(date('Y:m:d'));
          $parameter = array( 'p_mode' => 'Insert',
                              'a_EnumId' => 'null',
                              't_EnumTypeDescription' => $dmName,
                               'enumKey' =>$enumKey,
                               'b_ForAll' => 'null',
                               't_SettingValue' => $dmName,
                               'n_CreatedBy' => 'null',
      ...
      $dmId=$this->uri->segment('4');
      $this->load->view('layout/header');
      $parameter = array( 'dmName' => 'null',
                                  'd_CreatedOn' => 'null',
                                  'id' => $dmId,
                                  'act_mode' => 'editselect',
                                  'n_CreatedBy' => 'null',
