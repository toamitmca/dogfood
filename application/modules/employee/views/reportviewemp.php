...
<table>
  ...
  <tr>
    ...
    <th>ACTION</th>
     ...
     <?php  foreach ($expvoil as  $expvoil11) {  ?>
     	 <tr><td><?php echo $expvoil11->scat; ?></td><td><?php if($expvoil11->voi==1){ ?><a title="Your Amount is exceed from policy Report Max Amount" href="#" style="display: block; margin-left: 330px;" class="bug" id="vozxil"></a> <?php } else { ?> NO VOILATIONS<?php } ?></td></tr> 
   <?php  } ?></thead>
  <tbody>
     <tr>
...
<div>
	...
	<div class="colRow"><label>City</labeloverlay">
<div class="popup">
<a class="close_icon"></a>
<div class="buttonWrap">
	<a href="" class="loadbtn bluebg" id="save_extra_exp">Save</a></div>
...
<input type="text" name="report_name" disabled  id="report_name" value="<?php echo $claimreport->t_ReportName;?>" disabled /></div>
<div class="col">
	...
	<div class="col">
		...
		<select name="report_type" id="report_type" disabled>
...
?>
	<input type="text" name="status" id="status" value="<?php echo "Approved";  ?>" disabled>
<?php
	}else{
?>
	<input type="text" name="status" id="status" value="<?php echo $claimreport->n_status;  ?>" disabled >
<?php
	}
?>
...
<input type="text" disabled name="chaim_period_form" id="datepicker-example1" class="dat33 datepicker_all" value="<?php echo date('d M, Y', strtotime($claimreport->d_ClaimFrom)); ?>" ></div>
<div class="col"><label>Claim Period To</label> 
<input disabled id="datepicker-example1s" name="chaim_period_to" class="dat33" type="text" value="<?php echo date('d M, Y', strtotime($claimreport->d_ClaimTo)); ?>" ></div>
<div class="col"><label>Cash Advance <?php  if($curreny->CURRENCY == 'USD') { ?> $  <?php }else {?>
  <span class="WebRupee">Rs</span> <?php } ?></label>
<input disabled type="text" name="cash_advance" id="cash_advance" onkeypress="return isNumber(event)"; onkeyup = "return calculateTotalAmount();" value="<?php echo $claimreport->n_CashAdvance; ?>" /></div>
<div class="col">
...
<input disabled type="text" name="description" id="description" value="<?php echo $claimreport->t_ReportDesc;?>" /></div>
<div class="col">
...
?>
<span>
		<input type="button" class="loadbtn bluebg" name="" onclick=" return submitmynote(this.value)" value="Save Notes" >
...
?>
<span>	
<input type="button" class="loadbtn" name="" onclick=" return submitmynote(this.value)" value="Save Notes" >
...
<?php 
		...
		//echo $Myvalue->n_CategoriesID.'hello Aa gya Mai';
?> 	
...
?>
 <tr>
    <td><input type="checkbox" id="check_<?php echo $count;?>"></td>
        ...
        <td>
            <select  name="category[]" disabled  id="category_<?php echo $count;?>" class="category">
                <option value="">Select</option>
                <?php if(!empty($policyAssign)){
                    foreach ($policyAssign as $key => $value) {
						if($Myvalue->n_CategoriesID == $value->a_SpndngCatId){
							$selected = "selected";
						}else{
							$selected = "";
						}
					?>
                        <option <?php echo $selected; ?> value = "<?php echo $value->a_SpndngCatId; ?>"><?php echo $value->t_SpndName; ?></option>
                 <?php   }
                    }else{ echo "No Records Founds"; } ?>
            </select>
        </td>
        <td><input class="dat date datepicker_all" name="date[]" disabled type="text" value="<?php echo date('d M, Y', strtotime($Myvalue->d_Date)); ?>" ></td>
        <td><input disabled type="text" value="<?php echo $Myvalue->t_Amount; ?>"  class="amount1" id="amount_<?php echo $count;?>" name="amount[]" onkeypress="return isNumber(event)"; onkeyup="return calculateTotalAmount();" ></td>
        <td><input disabled type="text" name="merchant[]" class="merchant" value="<?php echo $Myvalue->t_Merchant; ?>" id="merchant_<?php echo $count;?>"></td>
        <td><input  disabled type="text" name="purpose[]" class="purpose" value="<?php echo $Myvalue->t_Purpose; ?>" id="purpose_<?php echo $count;?>"></td>
        <td><input disabled type="checkbox" name="reimb[]" class="reimb" id="reimb_<?php echo $count;?>" onclick="return checkReimb();" <?php if($Myvalue->b_IsReimburs==1){ echo "checked";}?> ></td>
        <td>
            <select name="tag[]" class="tag" id="tag_<?php echo $count;?>" disabled>
                <option>Yes</option>
                <option>No</option>
            </select>
        </td>
...
<a id="voil" class="bug" style="display: block; margin-left: 330px;" href="#" title="Click Here To See All Voilation"></a>
<?php
if($claimreport->n_status == 'submit' ||  $claimreport->n_status == 'Reimbursed'|| $claimreport->b_Approved==1){
?>
...
<div class="popupsecondh" id="atthFil">
		...
		<h2>Select Your File</h2>
		<div class="inputfile" id="popupFile"><input type="text" name="hidden" value="" id="check">
		<input class="boxtu" disabled type="text"  name="nametest" value="">
