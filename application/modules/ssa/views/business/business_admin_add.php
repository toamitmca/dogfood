...
<div class="rightSide">
	<div>
		<span class="buttonWrap">
			<!--  <a href="<?php  echo base_url(); ?>ssa/business/businessadmin" class="loadbtn bluebg">Add Admin</a>  -->
		</span>
		<div class="fix"></div>
	</div>
	<?php
	...
	<form name="detail_informaton" id="detail_informaton" method="post" action="<?php echo base_url(); ?>ssa/business/businessadmin/"> 
  <?php if($business !=="Something Went Wrong") { ?>
<div class="col"><label>Business name</label> 
		<select name="businessid" id="businessidby" onchange="return busdpmt();">
			<option value=''>Select a Status</option>
			<?php foreach ($business as $value9) { ?>
				<option value="<?php echo $value9->a_BusinessId; ?>" ><?php echo $value9->t_BusinessName; ?></option>
			<?php } ?>
		</select>
<?php } else { echo 'business not found';} ?>
	</div>
	...
	<div class="formPreExp">
		...
		<div class="col">
			...
			<select name="departmenr" id="bydepartment">
			 </select>
	...
	<tr><td>	<span><input type="checkbox" name="edit_policy[]" value="<?php echo $value1->a_RoleAccessId; ?>"> <?php echo $value1->t_AccessName; ?> </td><?php if($value1->a_RoleAccessId==5){?> <td><small><</small> <label>Rs. </label> <input style="width:30%;" type="text" name="amount" id="amount" value="<?php echo set_value('amount');?>"/> </td> <?php }?></span><tr>
	<?php } ?>
	</table>
	</div>
...
function busdpmt(){
	Mybase_url= base_url();
	businessid1 = $('#businessidby').val();
	console.log(businessid1);
    console.log(Mybase_url);

$.ajax({
          url: Mybase_url+'ssa/business/getdeparementbus/',
		  type: 'POST',
		  dataType:'json',
		  data: {'businessid':businessid1 },
    ...
    if(data !=="Something Went Wrong")
     {
           		$.each(data,function (index,value1){
           			var list1 = "<option value='"+value1.a_DeptId+"'>"+value1.t_DeptName+"</option>";
                 
				$("#bydepartment").append(list1);
				});
				  }
				  else{
				  	var list2 ="<option value=''> department not found </option>";
				  	
                    $("#bydepartment").append(list2);
				  }
